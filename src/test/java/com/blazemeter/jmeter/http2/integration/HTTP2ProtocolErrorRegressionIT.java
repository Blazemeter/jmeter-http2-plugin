...
public class HTTP2ProtocolErrorRegressionIT extends HTTP2TestBase {
  ...
  private static final String DEMOBLAZE_PATH = "/";
  ...
  public void shouldNotFallbackToHttp11OnDemoblaze() throws Exception {
    ...
    sampler.setDomain(DEMOBLAZE_HOST);
    sampler.setProtocol(HTTPConstants.PROTOCOL_HTTPS);
    sampler.setPath(DEMOBLAZE_PATH);

    HTTP2JettyClient client = new HTTP2JettyClient(false, "IT-Demoblaze");
    try {
      client.start();
      URL url = URI.create("https://www.demoblaze.com/").toURL();
